...
\begin{itemize}
  ...
  \item \texttt{InterpolatedPose} (cyan), driven by \texttt{DualQuaternionPoseDemo.cs}
    using \texttt{DualQuaternion.Slerp}.
  \item \texttt{DecoupledPose} (magenta), driven by plain \texttt{Vector3.Lerp} +
    \texttt{Quaternion.Slerp} for the Part C comparison.
...
\begin{itemize}
  ...
  \item \textbf{Orientation profile is identical here:} both methods compute
    ...
    quaternion) this coupling is precisely what a decoupled Lerp+Slerp
    cannot express.
  \item \textbf{Naturalness for VR:} the curved DLB path is consistent with a
    ...
    straight-chord path of separate interpolation looks mechanical/unnatural
    for a rotating hand or tool, since it is only correct when translation
    and rotation are physically independent.
...
\begin{itemize}[nosep]
  \item \texttt{Assets/Scripts/DualQuaternion.cs}, \texttt{DualQuaternionPoseDemo.cs},
    \texttt{Editor/SceneSetupHelper.cs}, \texttt{DualQuaternionMathTests.cs}.
  \item \texttt{tools/dualquat\_sim.py} --- numeric verification and Part C figure/table generator.
  \item This report (\texttt{report/main.tex}).
\end{itemize}
